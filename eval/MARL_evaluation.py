...
"""
@author: Hyunjun Choi
python MARL_evaluation.py --checkpoint-path ./results/ray_results/{experiment_name}/{checkpoint_xxx} --test-case HV_renew_case1 --target-country Germany
"""
...
from utils.env.MARL_env_evaluation import *
import pickle
import random
import pandas as pd
random.seed(2023)
...
parser.add_argument("--data-dir", type=str, default="./data", help="Directory containing test data files")
...
if __name__ == "__main__":
    ...
    def env_creator(env_config):
        env = PTX_env(env_config)
        return env

    register_env("PTX_env", env_creator)
    ...
    env_config['target_country'] = 'Germany'

    # Load trained policies
    try:
        ptx_policy_path = os.path.join(args.checkpoint_path, 'policies', 'PTX_policy')
        management_policy_path = os.path.join(args.checkpoint_path, 'policies', 'Manage_policy')
        
        Ptx_train = Policy.from_checkpoint(ptx_policy_path)
        man_train = Policy.from_checkpoint(management_policy_path)
        
        print(f"Loaded trained policies from {args.checkpoint_path}")
        
    except Exception as e:
        print(f"Error loading trained policies: {e}")
    ...
    # Action arrays
    ptx_action_stack = np.zeros(shape=(sample_size, args.op_period, 4))  # PTX_agent actions
    management_action_stack = np.zeros(shape=(sample_size, args.op_period, 2))  # Management_agent actions

    # Run evaluation
    for i in range(sample_size):
        print(f"Evaluating scenario {i+1}/{sample_size}")
        
        test_env = PTX_env(env_config)
        dones = {'__all__': False}
        obs = test_env.reset(selected_renewable[i], selected_smp[i], selected_demand[i], selected_p_demand[i])
        obs = obs[0]
        ...
        while not dones['__all__']:     
            ...
            for agent_id in obs:
                if agent_id == 'PTX_agent':
                    action, _, _ = Ptx_train.compute_single_action(obs[agent_id])
                    clipped_action = np.clip(action[0], test_env.action_space['PTX_agent'].low, test_env.action_space['PTX_agent'].high)
                    actions[agent_id] = clipped_action
                    ptx_action_stack[i, test_env.step_count, :] = clipped_action
                    
                elif agent_id == 'Management_agent':
                    action, _, _ = man_train.compute_single_action(obs[agent_id])
                    clipped_action = np.clip(action[0], test_env.action_space['Management_agent'].low, test_env.action_space['Management_agent'].high)
                    actions[agent_id] = clipped_action
                    management_action_stack[i, test_env.step_count, :] = clipped_action
            
            obs, rewards, dones, truncateds, infos = test_env.step(actions)
    ...
    results_path = os.path.join(args.results_dir, f'evaluation_results_{args.test_case}_{args.target_country}.pkl')
