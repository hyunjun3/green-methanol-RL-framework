...
class PTX_env_RL(gym.Env): #PtX power allocation model 
    ...
    def __init__(self, config = None):
        ...
        for i in range(2024-2015):
            year = i + 2015
            file_path  = os.path.join(path, 'data/{country}/building_demand_data/Demand_data_{year}.npy'.format(country = self.target_country, year=year))
        ...
        for i in range(2024-2015):
            year = i + 2015
            file_path  = os.path.join(path, 'data/{country}/building_demand_data/Pred_data_{year}.npy'.format(country = self.target_country, year=year))
        ...
        space_low = np.zeros(shape=(self.obs_length * 4 + 2 + 2), dtype=np.float32) # 24*4 + 2
        space_high = np.zeros(shape = (self.obs_length*4 + 2+2),dtype = np.float32) #  24*4 + 2
        space_high[:] = 1

        self.observation_space = spaces.Box(low = space_low, high = space_high, shape=(self.obs_length*4+4,), dtype=np.float32)     
        
        # action_space : ESS: [-1, 1], PEM[0,1], stored H2 utilization[0,1], H2 to market[0,1],  MEOH transport [0, 1], stored MEOH utilization[0,1]
        self.action_space = spaces.Box(low=np.array([-1,0,0,0,0,0],dtype = np.float32),
                                       high=np.array([1,1,1,1,1,1],dtype = np.float32), shape=(6,),dtype=np.float32)     
    ...
    def _RESET(self,renewable_data, SMP_data, demand_data, p_demand_data,  seed=None, options=None):
        self.step_count = 0
        self.action_acc = []
        self.ESS_charge = []
        self.ESS_discharge = []
        ...
        self.area1_MEOH_init = self.L_MEOH_1[0]
        self.area2_MEOH_init = self.L_MEOH_2[0]
        ...
        # renewable, SMP, SOC (%), H2 storage level(%), historical demand, predict demand, MEOH sotrage level(%) 1,2
        self.state = np.hstack([self.renewable[self.step_count:self.step_count+self.obs_length]/self.scale,
    ...
    def _STEP(self, action): #Action1 = 'ratio' of  ESS power & PEM power  & LH2 utilization & H2 split_fraction to market
                             #ACtion2 = MEOH transport [0, 1], stored MEOH utilization[0,1]  
                             
        self.action_acc.extend([action])
        ESS_action = action[0]*self.ESS_P_cap
        PEM_power_action = action[1]*self.PEM_P_cap 
        LH2_util = action[2]
        split_fraction =  action[3]
        transport_frac = action[4]
        LMEOH2_util = action[5]
        ...
        if H_mis>0:           
            if H_mis*self.SP_H2>PEM_power_action: 
                self.PEM_penalty += (H_mis*self.SP_H2 - PEM_power_action) #penalty (overdischarge)
        ...
        if self.step_count == len(self.renewable)-self.obs_length:
            ...
            self.normalized_grid_penalty_list.append(self.grid_penalty)
            ...
            if pred_end <= len(self.p_demand_data):
                pred_demand_normalized = (self.p_demand_data[pred_start:pred_end] - self.pred_min_demand) / (self.pred_max_demand - self.pred_min_demand)
            else:
                pred_demand_normalized = (self.p_demand_data[-24:] - self.pred_min_demand) / (self.pred_max_demand - self.pred_min_demand)
    ...
    def reset(self,renewable, SMP, demand_data, p_demand_data , seed=None, options=None):
        return self._RESET(renewable_data = renewable, SMP_data = SMP, demand_data = demand_data, p_demand_data = p_demand_data,seed=None, options=None)
...
env_config = {
    'X_flow': 864.9881513923024,
    'H2_storage_period': 2.65,
    'ESS_storage_period': 87.97,
    'fw': 0.5,
    'op_period': 720,
    ...
    'penalty_weigth': 1,
    'cost_weigth': 1,
    'MEOH_storage_period':17.595, #43.989726340141836, 17.595
}
...
for i in range(5):
    ...
    while not done:
        ...
        compare_list.append({
            ...
            "internal_pred": internal_pred.tolist(),
            "diff": np.abs(obs_pred - internal_pred).tolist()
        })
